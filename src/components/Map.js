...
const Map = () => {
  ...
  if (!meetingId) {
    return (
      <div className="relative min-h-screen bg-blue-50 flex flex-col items-center justify-center"> {/* flex-col 추가 */}
        <Header /> {/* 여기에도 Header를 추가할 수 있습니다 (선택 사항) */}
        <div className="flex flex-col items-center space-y-4">
          <div className="text-center text-red-600 font-bold text-lg">
            ❗ URL에 <code>?meetingId=abc123&&name=johnDoe</code> 를 붙여주세요.
          </div>
  ...
  return (
    <div className="bg-blue-50 min-h-screen relative"> {/* py-10 삭제 (Header가 여백을 가짐) */}
      <Header /> {/* Header 컴포넌트 추가 */}

      <h1 className="text-4xl font-bold text-center text-blue-800 mb-8 mt-8"> {/* mt-8 추가로 Header 아래 여백 */}
        {t('pageTitle')}
      </h1>

      {/* 언어 토글 버튼 위치 (오른쪽 상단 고정) */}
      {/* 헤더 아래로 이동해야 하므로 absolute top-4를 top-24 정도로 조정하거나, Header 컴포넌트 내에 포함하는 것을 고려 */}
      <div className="absolute top-24 right-4 z-10"> {/* Header 높이를 고려하여 top 값 조정 */}
        <button
          onClick={toggleLanguage}
          className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded shadow-md transition duration-200"
        >
      ...
      {/* 지도 및 컨트롤 패널 컨테이너 (기존 레이아웃 유지) */}
      <div className="max-w-6xl mx-auto bg-white shadow-xl rounded-2xl p-6 flex flex-col md:flex-row gap-6">

        {/* 지도 컨테이너 */}
        <div id="map" style={{ flex: '2', height: '500px', minWidth: '300px' }}>
          {/* 지도 자체는 Kakao Map API에 의해 여기에 그려집니다 */}
        </div>

        {/* 컨트롤 패널 (버튼 및 참가자 선택 등) 컨테이너 */}
        <div className="flex flex-col gap-4 flex-1 p-2">
      ...
      {/* 추천 위치 주소와 추천 리스트들을 지도 아래, 전체 너비로 확장 */}
      {(recommendAddress || Object.values(categoryPlaces).some(arr => arr.length > 0 && Object.keys(visibleCategories).some(key => visibleCategories[key]))) && (
        <div className="max-w-6xl mx-auto mt-6 bg-white shadow-xl rounded-2xl p-6 flex flex-col gap-6"> {/* md:flex-row 제거, flex-col 유지 */}
          {recommendAddress && (
            <div className="text-gray-700 text-lg p-2 bg-gray-50 rounded shadow-sm w-full"> {/* w-full 추가 */}
              <p className="mb-2">📍 추천 위치 주소: <span className="font-semibold">{recommendAddress}</span></p>
              <button
                onClick={handleCopyAddress}
                className="bg-blue-100 hover:bg-blue-200 text-blue-800 text-sm px-3 py-1 rounded shadow-sm hover:shadow transition duration-200 font-bold"
          ...
          {/* 카테고리별 추천 리스트들을 세로로 나열 */}
          {/* flex-wrap 제거, flex-col 유지 */}
          <div className="flex flex-col gap-4 w-full"> {/* w-full 추가 */}
            {Object.keys(categoryPlaces).map((key) => (
              visibleCategories[key] && categoryPlaces[key]?.length > 0 && (
                <div key={key} className="p-2 bg-gray-50 rounded shadow-sm w-full"> {/* w-full만 유지 */}
                  <h3 className="font-bold mb-2 text-gray-800">
                    {t('categoryListTitle', categoryConfigs[key].keyword)}
                  </h3>
                  <ul className="text-sm text-gray-700 list-disc ml-4">
